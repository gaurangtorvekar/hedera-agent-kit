...
import { BonzoGetATokenBalanceTool } from "./tools/bonzo/get_atoken_balance_tool";
...
export function createHederaTools(hederaKit: HederaAgentKit): Tool[] {
  const hederaTools = [
    ...
    new BonzoGetATokenBalanceTool(hederaKit),
  ];
  ...
  if (operatorId && operatorKey) {
    try {
      ...
      const standardsToolsArray = Object.values(standardsTools).filter(Boolean) as unknown as Tool[];
