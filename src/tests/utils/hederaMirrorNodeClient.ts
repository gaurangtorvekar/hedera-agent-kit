...
export class HederaMirrorNodeClient {
  ...
  async getTokenBalance(accountId: string, tokenId: string): Promise<number> {
    ...
    const balanceInDisplayUnit = parsedResponse?.balances[0] ? fromBaseToDisplayUnit(rawBalance as number, decimals as number) : 0;
  ...
  async getTransactionReport(transactionId: string, senderId: string, receiversId: string[]): Promise<txReport> {
    ...
    const totalFees = result.transactions[0].transfers.filter((t) => t.account !== senderId && !receiversId.find((r) => r === t.account)).reduce((sum, t) => sum + t.amount, 0);
  ...
  async getAllTokensBalances(accountId: string): Promise<Array<DetailedTokenBalance>> {
    let url: string | null = `${this.baseUrl}/balances?account.id=${accountId}`;
      ...
      while (url) {
        ...
        for (const token of data.balances[0]?.tokens || []) {
          ...
          const detailedTokenBalance: DetailedTokenBalance = {
            ...
            balanceInDisplayUnit: BigNumber(fromBaseToDisplayUnit(token.balance, +tokenDetails.decimals)),
          };
  ...
  async getAccountToken(accountId: string, tokenId: string): Promise<AccountToken | undefined> {
  ...
  async getAccountTokens(accountId: string): Promise<AccountToken[]> {
    ...
    let nextLink: string | null = `${this.baseUrl}/accounts/${accountId}/tokens?&limit=100&order=desc`;
    ...
    while (nextLink) {
      ...
      nextLink = data.links?.next ? `${this.baseUrl.replace("/api/v1", "")}${data.links?.next}` : null;
    }
